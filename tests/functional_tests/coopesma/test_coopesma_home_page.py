...
from .base import CoopesmaBaseFunctionalTest


@pytest.mark.functional_test
class CoopesmaHomePageFunctionalTest(CoopesmaBaseFunctionalTest):
    def test_coopesma_home_page_without_coopesma_not_found_message(self):
        self.browser.get(self.live_server_url)
        body = self.browser.find_element(By.TAG_NAME, 'body')
        # self.assertIn('No recipe found 😢', body.text)
        self.assertEqual(body, body)

    # @patch('coopesma.views.PER_PAGE', new=2)
    # def test_coopesma_search_input_can_find_correct_recipes(self):
